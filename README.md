...
Measures the *clean post-training overconfidence temperature* by running the **pre-trained (base)** and **post-trained** variants of seven open models on the AIReg-Bench validation cells (independent human GT), to decide whether a **transferred constant temperature** can correct closed-evaluator (**Anthropic + GPT**) overconfidence on out-of-sample documents — where DACA, internal-dispersion, and accuracy-contaminated supervised fits all failed. (The collaborative-evaluation pair switched back to Anthropic + GPT on 2026-07-02, freeing Google to join the annotators — Gemma-4-26B-A4B is the seventh.)
...
> discarded**; the **paid vast LIVE study** (vLLM, bf16, all 120 cells, reasoning ON, the seven panel
...
- **Post leg:** the **seven** post-trained panel models via API from local macOS (Claude Code orchestrates).
- **Analysis:** reuses `judex-evaluator` tooling verbatim — `judex.calibration` (`fit_temperature`, `apply_temperature`, `fit_dispersion_temperature`) and `judex.experiments` (`murphy_decomposition` — defined in `judex.metrics_report` since 2026-07-18 and re-exported, `dispersion_calibration_recovery`) — scored against the **canonical, manifest-verified** AIReg GT (`aireg.py` synthesizes it from the git-tracked `judex-ground-truth` bundle; no run dependency). Temperatures are fit on **Study A's own** `T_BOUNDS = (0.25, 20.0)`, passed explicitly — the evaluator's default `(0.25, 4.0)` would censor them. See [`docs/integration_remediation_2026_07_01.md`](docs/integration_remediation_2026_07_01.md), whose **2026-07-18 addendum** is the current cross-repo state (the body and the 07-03 addendum are historical).
